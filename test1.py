...
def generate_file_location(date, option):
    ...
    return f"{BASE_DIRECTORY}/{formatted_date}/{option}/summary.csv"
...
def on_scroll(event):
    ...
    scale_factor = 1.2 if event.button == 'up' else 1 / 1.2  # Adjust the zoom level as needed
    new_ylim = [x * scale_factor for x in current_ylim]
...
def plot_data(file_location, columns, chart_type, timezone):
    ...
    fig, axs = plt.subplots(num_rows, num_cols, figsize=(15, 3 * num_rows), sharex=False, squeeze=False)
    ...
    for i, (attr, is_checked) in enumerate(attributes_to_plot):
        if is_checked:
            row = current_subplot // num_cols
            col = current_subplot % num_cols
            ax = axs[row, col]
            ...
            if chart_type == 'plot':
                ...
                ax.scatter(x, y, label=attr)
            elif chart_type == 'bar':
                ax.bar(x, y, label=attr)
    ...
    # Connect the 'motion_notify_event' to the scroll function
    fig.canvas.mpl_connect('motion_notify_event', on_scroll)
