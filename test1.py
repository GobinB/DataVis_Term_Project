...
def draw_figure(canvas, figure, loc=(0, 0)):
    # First, destroy the previous figure and its frame to avoid duplication
    if hasattr(canvas, 'scrollable_frame'):
        canvas.scrollable_frame.destroy()

    # Create a new frame to contain the canvas and scrollbar
    scrollable_frame = tk.Frame(canvas.master)
    canvas.scrollable_frame = scrollable_frame
    scrollable_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

    # Create the FigureCanvasTkAgg object within the scrollable frame
    figure_canvas_agg = FigureCanvasTkAgg(figure, master=scrollable_frame)
    figure_canvas_agg.draw()
    widget = figure_canvas_agg.get_tk_widget()
    widget.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

    # Create vertical scrollbar and attach it to the widget within the frame
    vscrollbar = ttk.Scrollbar(scrollable_frame, orient='vertical', command=widget.yview)
    vscrollbar.pack(side='right', fill='y')

    # Configure the canvas widget's yscrollcommand to the scrollbar
    widget.configure(yscrollcommand=vscrollbar.set)

    # Update canvas.figure_agg
...
def main():
    ...
    window = sg.Window('Data Analysis App', layout, size=(1400, 950), resizable=True)
